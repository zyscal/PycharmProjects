...
Client_RTT_std = initarray(10, 30)
Client_RTT_std_total = initarray(10, 30)
Server_RTT_std = initarray(10, 30)
Server_RTT_std_total = initarray(10, 30)
...
for i in datas:
    ...
    this_Client_RTT_std = int(i[4])
    this_Server_RTT_std = int(i[5])
    ...
    Client_RTT_std_total[index] += this_Client_RTT_std * this_Client_RTT_std * this_times
    Server_RTT_std_total[index] += this_Server_RTT_std * this_Server_RTT_std * this_times

for i in range(10, 31):
    index = i - 10
    Client_RTT_std[index] = (Client_RTT_std_total[index] / test_times[index]) ** 0.5
    Server_RTT_std[index] = (Server_RTT_std_total[index] / test_times[index]) ** 0.5
...
data = [Client_RTT_std, Server_RTT_std]
...
def create_multi_bars(labels, datas, tick_step=1, group_gap=0.2, bar_gap=0):
    ...
    for index, y in enumerate(datas):
        ...
        if i == 0:
            plt.bar(baseline_x + index*bar_span, y, bar_width, color='b', hatch='////', label = "Client RTT std", alpha=0.7)
        elif i == 1:
            plt.bar(baseline_x + index*bar_span, y, bar_width, color='g',hatch='\\\\\\\\', label = "Server RTT std",alpha=0.7)
        i += 1
        plt.ylabel('ms')
        plt.xlabel('QPS')
        plt.title('LwM2M/TCP/NB-IoT RTT_STD-QPS')
        # x轴刻度标签位置与x轴刻度一致
