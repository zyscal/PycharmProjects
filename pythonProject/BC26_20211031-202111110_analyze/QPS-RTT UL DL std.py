...
def create_multi_bars(labels, datas, tick_step=1, group_gap=0.2, bar_gap=0):
    ...
    for index, y in enumerate(datas):
        ...
        if i == 0:
            plt.bar(baseline_x + index*bar_span, y, bar_width, color='y', hatch='///', label = "UL delay std", alpha=0.7)
        elif i == 1:
            plt.bar(baseline_x + index*bar_span, y, bar_width, color='purple',hatch='\\\\\\', label = "DL delay std",alpha=0.7)
        i += 1
        plt.ylabel('ms')
        plt.xlabel('QPS')
        plt.title('LwM2M/TCP/NB-IoT UL_DL_std-QPS ')
        # x轴刻度标签位置与x轴刻度一致
